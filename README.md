...
* [ ] **當事人 (`:Person`) 節點之同名同姓實體混淆與去重** (次級/待評估)：評估是否需改用與案件 ID 綁定的當事人表示法，以避免因同名同姓被錯誤合併而造成的圖譜虛假連結。
* [ ] **法官與律師超級節點 (`Supernodes`) 查詢效能優化** (次級/待評估)：評估是否需在圖走訪與社群偵測查詢中，主動限制或過濾法官與律師等高連通度節點，以降低計算負載並提升查詢響應速度。
* [ ] **法規 (`:Law`) 節點之階層化/樹狀重構** (次級/待評估)：規劃將扁平的 Law 節點改為 `(:Statute)-[:HAS_CHAPTER]->(:Chapter)-[:HAS_ARTICLE]->(:Article)` 的多層級階層設計，以支援上位法規的泛化與推理檢索。
* [ ] **案件審級與上訴歷程關係 (`:APPEALED_FROM` / `:REVERSED` 關係)** (次級/待評估)：研議是否建立判決書與判決書之間的上下級審級關聯，以利在圖中追蹤相同訴訟的審判演變路徑。
